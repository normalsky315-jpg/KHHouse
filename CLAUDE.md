...
10. **同一個真實建案可以「跨區列在兩個行政區頁面」，但目前 schema 沒有多區欄位（`district`
    只能是單一值）**——做法是把整筆資料完整複製一份，第二份只改 `district`，其他地址/
    價格/格局/建材/公設/notes 都跟原始那份一樣（因為物理上就是同一棟房子）。目前有兩個
    案例：
    - 「靚CASA」：`addr` 寫鳳山區過勇路312號，但 `district` 是「大寮」——只有一份紀錄，
      不是複製，是原本就這樣分類（大寮/鳳山交界，行政區跟生活圈标示不同）。
    - 「清朗」：2026-09-09 使用者要求把鳳山區的「清朗」也加進大寮頁面，兩邊都要顯示。
      做法是複製一份完整資料（地址仍寫鳳山區誠體街，`district` 改成「大寮」），並在複製
      版本的 `notes` 結尾加一句話說明是跨區收錄、資料以鳳山版本為準。**這兩筆是各自獨立
      的 data.json 項目，以後改其中一筆的價格/資料，不會自動同步到另一筆**——如果使用者
      提供「清朗」的更新資料，要記得問清楚是只改一邊還是兩邊都要改，兩邊都要改的話要
      手動改兩次。這也代表「全部案」的總數/統計會把清朗算兩次（87 案裡有 2 筆同名的
      「清朗」），這是使用者知情選擇的結果，不是重複資料的 bug。
11. **楠梓/仁武（高大特區，index.html 顯示）的實登資料非常久沒更新了**：用 git log 比對
    確認過，楠梓 20 案從 2026-07-02（初版）、仁武 20 案從 2026-07-03 15:51（三來源資料
    彙整那次）之後，avgPrice/count 完全沒改過（2026-09-09 查核時已經兩個多月）。這些
    專案**沒有 `dataPeriod` 欄位**（只有這幾天新處理的大寮案子才有），commit 訊息也沒寫
    當初查的實際區間，所以問不出「原本是抓幾月到幾月」，只能推斷「近兩年」是頁面固定文字
    的宣稱、不是存下來的精確窗口。使用者如果要更新楠梓/仁武資料，建議照大寮這次的做法
    （抓一份內政部實價登錄 Excel，起始日期用當時日期往前推約兩年，並存 `dataPeriod` +
    `transactions`），不要再猜「原本用的是什麼區間」。
12. **「遠雄蘴靚」（2026-09-09 新增，楠梓）跟「遠雄一靚」是兩個完全不同的建案**，都是遠雄
    建設在楠梓高大特區的案子，很容易搞混：
    - 遠雄一靚：大學二十三街vs大學二十六街，2432坪基地，20F/B3，461戶，已有實登均價
      42.46萬/坪（61筆）
    - 遠雄蘴靚：惠正街vs惠勇街（惠民段16地號），3795坪基地（楠梓史上最大單一案），
      22F、4棟ㄇ字型配置，686戶，2026年才剛預售，開價34~39萬/坪，尚無實登成交
    使用者最初打成「遠雄豐靚」——正確的字是「蘴」不是「豐」（字形相近，591/樂居/新聞稿
    上都是用「蘴」），案名要打對字，不要因為使用者打錯字就自動改成同音的「豐靚」存進去。
    這個案子太新，591 只有建案總覽頁（`newhouse.591.com.tw/141105`），查不到樂居/958
    專頁，也查不到建照號、貸款成數、交屋時間、確切公設比，這些欄位先留空，不要用「遠雄
    一靚」的數字去填補（兩個案子公設比、車位數等完全不同，套錯會誤導）。
